...
Also, the sheer number of paths that was returned was so long that it lengthened the document by at least a dozen pages so all the ```...``` in the output is actual a range of 2 - 50 files that I cut out to minimize the page amount. Twenty-five pages was not downloading so believe me when I say it's for your beneift. With that in mind, the actual output of the command with a very broad path is immense. So this command is best used when the results can be limited to a specific range/can be limited in some way.
